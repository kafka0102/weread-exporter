...
- 章切换后按该章字数动态等待：`ceil(字数/2000)*SLEEP_CHAPTER_PER_2K_CHARS`，夹在 `SLEEP_CHAPTER_MIN`–`SLEEP_CHAPTER_MAX`（默认 0.3–2 秒）
...
```
export_precise.py:
  浏览器登录 → 打开目录抓取章名列表（仅开篇/续传时点一次目录）
    → 键盘 ArrowRight / 点阅读区中右 线性逐页翻（中途不再频繁点目录）
    → 每页: Canvas Hook 捕获文字 + 视口内图片 URL
    → 按正文/顶栏切章；长章持续翻页抓取，仅停滞时重开阅读器续传
    → 翻到目录最后一章自动停止
