...
1. **单一配置源**：新增或修改浏览器节奏时，在 `env_config.py` 增加/使用 `SLEEP_*` 常量，默认值与 `.env.example` 保持一致。
2. **本地可调**：开发者复制 `.env.example` → `.env` 后改数值即可；`.env` 不入库（见 `.gitignore`）。进程环境变量优先于 `.env` 文件。
3. **CLI 可覆盖**：若已有命令行参数（如 `--sleep`、`--author-interval`），参数优先于 `.env` 默认值。
4. **文档同步**：增删 `SLEEP_*` 时同步更新 `.env.example`、本文件下表，以及 README 中相关说明。
